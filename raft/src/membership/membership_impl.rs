  ...
  N: Node,
{
    ...
    N: Default,
  {
    ...
    let nodes = Self::extend_nodes(
      nodes.into_iter().map(|x| (x, N::default())).collect(),
      &voter_nodes,
    );
  ...
  N: Node,
{
  ...
  pub(crate) fn ensure_valid(&self) -> Result<(), MembershipError<NID>> {
    ...
    self
      .ensure_voter_nodes()
      .map_err(|nid| NodeNotFound::new(nid, Operation::None))?;
    Ok(())
  }
  ...
  pub(crate) fn is_direct_append_compatible_with(&self, other: &Self) -> bool {
    ...
    if both_uniform {
      let differing = self.configs[0]
        .symmetric_difference(&other.configs[0])
        .count();
      return differing <= 1;
    }
  ...
  pub(crate) fn change(
    ...
    log::debug!("{}: change: {:?}", func_name!(), change);
    ...
    log::debug!("new membership: {}", new_membership);
...
mod tests {
  ...
  use crate::{
    ...
    errors::{ChangeMembershipError, EmptyMembership, LearnerNotFound, MembershipError},
    type_config::alias::ChangeMembershipErrorOf,
  };
  ...
  fn test_membership_change() -> anyhow::Result<()> {
    ...
    // Add: no such learner
    {
      ...
      let err: ChangeMembershipErrorOf<UTConfig> = res.unwrap_err().into();
      assert_eq!(
