...
fn test_membership_summary() -> anyhow::Result<()> {
    ...
    let m = Membership::<u64, ()>::new_with_defaults(
...
fn test_membership_new_unchecked() -> anyhow::Result<()> {
    let node = TestNode::default;
    let new_unchecked = |nodes| {
        Membership::<u64, TestNode>::new_unchecked(vec![btreeset! {1}, btreeset! {2}], nodes)
    };
...
fn test_membership_ensure_quorum_defined() -> anyhow::Result<()> {
    ...
    let empty_set =
        Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2}, btreeset! {}], []);
...
fn test_membership_is_direct_append_compatible_with_joint() -> anyhow::Result<()> {
    ...
    let two_sets_grown = Membership::<u64, ()>::new_with_defaults(
        vec![btreeset! {1,2,3,7}, btreeset! {4,5,6,8}],
        [],
    );
