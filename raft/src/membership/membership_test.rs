...
use crate::{
  ...
  errors::{EmptyMembership, MembershipError, NodeNotFound, Operation},
};
...
fn test_membership_summary() -> anyhow::Result<()> {
  ...
  let m =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2}, btreeset! {3}], btreeset! {4});
  assert_eq!(
    "{voters:[{1:(),2:()},{3:()}], learners:[4:()]}",
    m.to_string()
  );

  let m = Membership::<u64, TestNode>::new_unchecked(
...
fn test_membership() -> anyhow::Result<()> {
  ...
  let m123_345 =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2,3}, btreeset! {3,4,5}], []);
...
fn test_membership_with_learners() -> anyhow::Result<()> {
  // test multi membership with learners
  {
    ...
    let m1_23 = m1_2
      .clone()
      .change(ChangeMembers::AddNodes(btreemap! {3=>()}), true)?;
    ...
    let m = m1_23
      .clone()
      .change(ChangeMembers::AddNodes(btreemap! {1=>()}), true)?;
  ...
  {
    let s1_2 = Membership::<u64, ()>::new_with_defaults(
      vec![btreeset! {1,2,3}, btreeset! {5,6,7}],
      btreeset! {3,4,5},
    );
...
fn test_membership_add_learner() -> anyhow::Result<()> {
  ...
  let res = m_1_2
    .clone()
    .change(ChangeMembers::AddNodes(btreemap! {1=>node("3")}), true)?;
  assert_eq!(
...
fn test_membership_update_nodes() -> anyhow::Result<()> {
  ...
  let m_1_2_3 = m_1_2.change(
    ChangeMembers::SetNodes(btreemap! {2=>node("20"), 3=>node("30")}),
    true,
  )?;
  assert_eq!(
    Membership::<u64, TestNode>::new_unchecked(
...
fn test_membership_new() -> anyhow::Result<()> {
  ...
  let with_nodes =
    |nodes| Membership::<u64, TestNode>::new(vec![btreeset! {1}, btreeset! {2}], nodes);
...
fn test_membership_new_unchecked() -> anyhow::Result<()> {
  let node = TestNode::default;
  let new_unchecked =
    |nodes| Membership::<u64, TestNode>::new_unchecked(vec![btreeset! {1}, btreeset! {2}], nodes);
...
fn test_membership_next_coherent() -> anyhow::Result<()> {
  ...
  let m23_with_learners_old =
    Membership::<u64, ()>::new_with_defaults(vec![c2(), c3()], old_learners());
  let m23_with_learners_new = Membership::<u64, ()>::new_with_defaults(vec![c3()], learners());
  assert_eq!(
    m23_with_learners_new,
    m23_with_learners_old.next_coherent(c3(), true)
  );
...
fn test_membership_next_coherent_with_nodes() -> anyhow::Result<()> {
  ...
  let initial = Membership::<u64, TestNode>::new_unchecked(
    vec![c1(), c2()],
    btreemap! {1=>node("1"), 2=>node("2")},
  );
...
fn test_membership_ensure_valid() -> anyhow::Result<()> {
  ...
  assert_eq!(
    Err(MembershipError::EmptyMembership(EmptyMembership {})),
    res
  );
...
fn test_membership_is_direct_append_compatible_with_joint() -> anyhow::Result<()> {
  ...
  // shared.
  let overlapping =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2,3}, btreeset! {1,2,4}], []);
  ...
  let two_sets =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2,3}, btreeset! {4,5,6}], []);
  let two_sets_grown =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2,3,7}, btreeset! {4,5,6,8}], []);
...
fn test_membership_is_direct_append_compatible_with_invalid_config() -> anyhow::Result<()> {
  ...
  // `{1,2,3}` with `previous`.
  let empty_set =
    Membership::<u64, ()>::new_with_defaults(vec![btreeset! {1,2,3}, btreeset! {}], []);
...
fn test_membership_find_changed_node_metadata() -> anyhow::Result<()> {
  ...
  assert_eq!(
    None,
    previous.find_changed_node_metadata(&removed_and_added)
  );
  ...
  assert_eq!(
    Some(3),
    previous.find_changed_node_metadata(&learner_changed)
  );
