...
use crate::{
  LogIdOptionExt, RaftTypeConfig,
  base::shared_id_generator::SharedIdGenerator,
  engine::leader_log_ids::LeaderLogIds,
  errors::QuorumNotEnough,
  progress::{VecProgress, entry::ProgressEntry, id_val::IdVal, stream_id::StreamId},
  quorum::QuorumSet,
  type_config::{
    TypeConfigExt,
    alias::{CommittedLeaderIdOf, CommittedVoteOf, InstantOf, LogIdOf},
  },
  ...
  QS: QuorumSet<Id = C::NodeId> + Clone + fmt::Debug + 'static,
{
  ...
  pub(crate) fn is_lease_valid(&self, leader_lease: Duration) -> bool {
    ...
    self
      ...
      .is_some_and(|acked| now < acked + leader_lease)
  }
  ...
  pub(crate) fn is_replication_stream_valid(
    &self,
    target: &C::NodeId,
    stream_id: StreamId,
  ) -> bool {
...
mod tests {
  ...
  fn test_leader_new_with_proposed_log_id() {
    log::info!("--- vote greater than last log id, create new noop_log_id");
    {
      ...
      let leader = Leader::<UTConfig, _>::new(
        ...
        Some(LeaderLogIds::new(
          *log_id(1, 2, 0).committed_leader_id(),
          1,
          3,
        )),
    ...
    log::info!("--- vote equals last log id, reuse noop_log_id");
    {
    ...
    log::info!("--- vote equals last log id, reuse noop_log_id, last_leader_log_id.len()==1");
    {
      let vote = Vote::new(1, 2).to_committed();
      let leader = Leader::<UTConfig, _>::new(
        vote,
    ...
    log::info!("--- no last log ids, create new noop_log_id, last_leader_log_id.len()==0");
    {
      let vote = Vote::new(1, 2).to_committed();
      let leader = Leader::<UTConfig, _>::new(
        vote,
  ...
  fn test_multiple_entries() {
    ...
    assert_eq!(
      log_ids,
      vec![log_id(2, 2, 9), log_id(2, 2, 10), log_id(2, 2, 11)]
    );
  ...
  fn test_clock_quorum_not_enough() {
    ...
    assert_eq!(
      btreeset! {1},
      err.got,
      "an RPC sent at t1 is not newer than t1"
    );
