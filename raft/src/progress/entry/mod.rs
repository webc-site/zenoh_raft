...
use crate::{
  ...
  engine::EngineConfig,
  progress::{
    VecProgressEntry, VecProgressEntryData, entry::update::Updater, inflight::Inflight,
    stream_id::StreamId,
  },
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) fn next_send(
    ...
    if is_probing {
      ...
      let start = max(mid, purge_upto_next);
      let end = min(start + max_entries, last_next);
