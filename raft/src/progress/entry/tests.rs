use crate::{
  LogId, RaftState,
  engine::{EngineConfig, LogIdList, testing::UTConfig},
  progress::{
    entry::ProgressEntry, inflight::Inflight, inflight_id::InflightId, stream_id::StreamId,
  },
  type_config::alias::{LeaderIdOf, LogIdOf, SnapshotMetaOf, StoredMembershipOf},
  vote::RaftLeaderId,
};
...
fn test_is_log_range_inflight() -> anyhow::Result<()> {
  ...
  assert!(!pe.is_log_range_inflight(&log_id(2)));

  pe.data.inflight = inflight_logs(2, 4);
  assert!(!pe.is_log_range_inflight(&log_id(1)));
  assert!(!pe.is_log_range_inflight(&log_id(2)));
  assert!(pe.is_log_range_inflight(&log_id(3)));
  ...
  assert!(!pe.is_log_range_inflight(&log_id(2)));
  assert!(pe.is_log_range_inflight(&log_id(3)));
  assert!(pe.is_log_range_inflight(&log_id(100)));
...
fn test_update_matching() -> anyhow::Result<()> {
  ...
  // Update matching and inflight
  {
    ...
    pe.new_updater(&engine_config)
      .update_matching(Some(log_id(6)), Some(InflightId::new(0)));
    assert_eq!(inflight_logs(6, 10), pe.data.inflight);
    assert_eq!(Some(&log_id(6)), pe.matching());
    assert_eq!(20, pe.data.searching_end);

    pe.new_updater(&engine_config)
      .update_matching(Some(log_id(10)), Some(InflightId::new(0)));
  ...
  // `searching_end` should be updated
  {
    ...
    pe.new_updater(&engine_config)
      .update_matching(Some(log_id(20)), Some(InflightId::new(0)));
...
fn test_update_conflicting() -> anyhow::Result<()> {
  ...
  let engine_config = EngineConfig::new_default(1);
  pe.new_updater(&engine_config)
    .update_conflicting(5, Some(InflightId::new(0)));
...
fn test_next_send_not_pipeline_when_gap_exists() -> anyhow::Result<()> {
  ...
  let res = pe.next_send(&mut new_raft_state(6, 10, 20), 100);
  assert_eq!(
    Ok(&inflight_logs(7, 20).with_id(1)),
    res,
    "not pipeline: gap exists"
  );
