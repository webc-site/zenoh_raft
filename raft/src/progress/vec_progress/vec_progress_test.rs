use std::collections::{BTreeMap, BTreeSet, btree_set::IntoIter};

use maplit::btreeset;

use super::VecProgress;
use crate::{
  progress::{VecProgressEntry, VecProgressEntryData},
  quorum::QuorumSet,
};
  ...
  QS: QuorumSet<Id = u64>,
{
  ...
  for candidate in candidates {
    let ids = values
      .iter()
      .filter_map(|(id, val)| (*val >= candidate).then_some(id));
...
fn vec_progress_display_with() {
  ...
  let display = progress.display_with(|f, item| {
