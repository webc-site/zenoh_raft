...
use crate::{
  RaftNetworkFactory, RaftTypeConfig,
  base::BoxStream,
  display_ext::display_instant::DisplayInstantExt,
  errors::{RPCError, ReplicationClosed},
  log_id_range::LogIdRange,
  network::{NetBackoff, NetStreamAppend, RPCOption},
  progress::inflight_id::InflightId,
  raft::{AppendEntriesRequest, StreamAppendError, StreamAppendResult},
  raft_state::IOId,
  replication::{
    backoff_state::BackoffState, event_watcher::EventWatcher,
    inflight_append_queue::InflightAppendQueue, replication_context::ReplicationContext,
    stream_context::StreamContext,
  },
  storage::RaftLogStorage,
  type_config::{
    TypeConfigExt,
    alias::{InstantOf, JoinHandleOf, MutexOf},
  },
};
  ...
  async fn run_stream_session(
    ...
    let resp_strm = match resp_strm_res {
      ...
      Err(rpc_err) => {
        ...
        self
          .send_progress_error(rpc_err, "initiate-stream-replication")
          .await;
  ...
  async fn handle_response_stream<'s>(
    ...
    loop {
      ...
      let append_res = match rpc_res {
        ...
        Err(rpc_err) => {
          self
            .send_progress_error(rpc_err, "stream-replication")
            .await;
          return Err("RPCError");
        }
      };
        ...
        Err(append_err) => {
          match append_err {
            StreamAppendError::Conflict(conflict_log_id) => {
              self
                .notify_progress(ReplicationResult(Err(conflict_log_id)))
                .await;
            }
