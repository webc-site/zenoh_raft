...
use crate::{
  Config, RaftTypeConfig, StorageError,
  core::notification::Notification,
  progress::{inflight_id::InflightId, stream_id::StreamId},
  raft_state::IOId,
  replication::response::{Progress, ReplicationResult},
  type_config::alias::{CommittedVoteOf, InstantOf, MpscSenderOf, VoteOf, WatchReceiverOf},
  vote::raft_vote::RaftVote,
};
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) fn leader_changed(&self, accepted_io: &IOId<C>) -> bool {
    ...
    log::info!(
      "{self}: Leader changed from {belonging_leader} to {current_leader}, quit replication"
    );
    true
  }
  ...
  pub(crate) async fn notify_storage_error(&self, error: StorageError<C>) {
    self
      .tx_notify
      .send(Notification::<C>::StorageError { error })
      .await
      .ok();
  }
  ...
  pub(crate) async fn notify_heartbeat_progress(&self, sending_time: InstantOf<C>) {
    self
      .tx_notify
      .send(Notification::<C>::HeartbeatProgress {
  ...
  pub(crate) async fn notify_progress(
    ...
    self
      .tx_notify
      .send(Notification::<C>::ReplicationProgress {
        stream_id: self.stream_id,
        progress: Progress {
          target: self.target.clone(),
          result,
  ...
  /// Report that the target has a higher vote than the leader.
  pub(crate) async fn notify_higher_vote(&self, higher: VoteOf<C>) {
    self
      .tx_notify
      .send(Notification::<C>::HigherVote {
        target: self.target.clone(),
        higher,
  ...
  C: RaftTypeConfig,
{
  fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
    write!(
      f,
      "{{id: {}, target: {}, {}}}",
      self.id, self.target, self.stream_id
    )
  }
}
