...
use crate::{
  RaftTypeConfig, errors::RPCError, network::Backoff,
  replication::backoff_consumer::BackoffConsumer,
};
...
mod tests {
  ...
  fn on_error_accumulates_rank_and_reconcile_enables_backoff() {
    ...
    assert!(
      state.is_enabled(),
      "backoff should be enabled after error > threshold"
    );
  ...
  fn observe_dispatches_to_on_success_or_on_error() {
    use crate::{engine::testing::UTConfig, errors::Unreachable};

    let mut state = BackoffState::new();

    // Err arm: accumulates rank using the error's `backoff_rank` (100 for Unreachable).
    let err: Result<(), RPCError<UTConfig>> = Err(RPCError::Unreachable(
      Unreachable::<UTConfig>::from_string("x"),
    ));
    state.observe(&err);
    assert_eq!(
      state.rank(),
      100,
      "err arm: rank accumulated from Unreachable weight"
    );
