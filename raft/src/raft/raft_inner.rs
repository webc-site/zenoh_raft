...
use crate::{
  Config, Extensions, OptionalSend, RaftMetrics, RaftTypeConfig,
  config::RuntimeConfig,
  core::{
    TickHandle,
    io_flush_tracking::IoProgressWatcher,
    raft_msg::{RaftMsg, external_command::ExternalCommand},
  },
  errors::Fatal,
  metrics::{RaftDataMetrics, RaftServerMetrics, Wait},
  raft::core_state::CoreState,
  type_config::{
    TypeConfigExt,
    alias::{MpscSenderOf, OneshotReceiverOf, OneshotSenderOf, WatchReceiverOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) async fn send_msg(&self, mes: RaftMsg<C>) -> Result<(), Fatal<C>> {
    ...
    if let Err(e) = send_res {
      ...
      log::error!("failed to send RaftMsg to RaftCore: {msg}, error: {fatal}",);
      return Err(fatal);
    }
    Ok(())
  }
  ...
  {
    let sum = if log::log_enabled!(Debug) {
      Some(mes.to_string())
    } else {
      None
    };
    ...
    self.recv_msg(rx).await.inspect_err(|_| {
      log::error!("failed to receive from RaftCore: {}", sum.display());
    })
  }
    ...
    match recv_res {
      ...
      Err(_) => {
        ...
        let fatal = if self.wait_core_stopped(RECV_CORE_STOP_TIMEOUT).await {
          let fatal = self.get_core_stop_error().await;
          log::error!("{}: error: {}", func_name!(), fatal);
          fatal
        } else {
          log::error!(
  ...
  pub(in crate::raft) async fn join_core_task(&self) {
    ...
    let running_res = {
      ...
      match &*state {
        CoreState::Running(_) => {
          ...
          let prev = mem::replace(&mut *state, CoreState::Joining(rx));
    ...
    match running_res {
      Ok((join_handle, tx)) => {
        ...
        log::info!("RaftCore exited: {:?}", join_res);
