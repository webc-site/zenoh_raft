...
use crate::{
  ChangeMembers, Membership, Raft, RaftTypeConfig,
  batch::Batch,
  errors::{ClientWriteError, RaftError, into_raft_result::IntoRaftResult},
  raft::{
    ChangeMembershipOutcome, ChangeMembershipRequest, ClientWriteResponse, ManagementApi,
    Precondition,
  },
  storage::RaftStateMachine,
  type_config::alias::BatchOf,
};
  ...
  SM: RaftStateMachine<C>,
{
  ...
  pub async fn change_membership_with_payload(
    ...
    ManagementApi::new(self.inner.as_ref())
      .change_membership_with_payload(request)
      .await
      .into_raft_result()
  }
