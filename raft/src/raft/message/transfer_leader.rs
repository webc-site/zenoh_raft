    ...
    C: RaftTypeConfig,
{
    ...
    #[error(
        "transfer leader rejected because vote changed; expected: {expected:?}, actual: {actual:?}"
    )]
    VoteChanged {
    ...
    #[error(
        "transfer leader rejected because log is not flushed; expected: {expected:?}, actual: {actual:?}"
    )]
    LogNotFlushed {
