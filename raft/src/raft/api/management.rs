...
use crate::{
  ChangeMembers, LogIdOptionExt, Membership, RaftMetrics, RaftTypeConfig,
  batch::Batch,
  core::{
    raft_msg::{RaftMsg, membership_payloads::MembershipPayloads},
    replication_lag,
  },
  entry::RaftPayload,
  errors::{ClientWriteError, Fatal, InitializeError},
  impls::ProgressResponder,
  membership::IntoNodes,
  raft::{
    ChangeMembershipOutcome, ChangeMembershipRequest, ClientWriteResult, Precondition,
    raft_inner::RaftInner,
  },
  type_config::{
    TypeConfigExt,
    alias::{BatchOf, LogIdOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) async fn change_membership_with_payload(
    ...
    log::debug!("change_membership: start",);
    ...
    let resp = match client_write_result {
      Ok(x) => x,
      Err(e) => {
        log::error!("the first step error: {}", e);
        return Ok(Err(e));
      }
    };

    log::debug!("res of first step: {}", resp);
    ...
    log::debug!("committed a joint config: {} {:?}", log_id, membership);
    log::debug!(
    ...
    if let Err(e) = &client_write_result {
      log::error!("the second step error: {}", e);
    }
  ...
  pub(crate) async fn append_membership(
    ...
    log::info!("append_membership: proposed membership: {}", membership);
  ...
  pub(crate) async fn add_learner(
    ...
    let wait_res = self
      ...
      .metrics(
        |metrics| {
          self
            .check_replication_upto_date(metrics, &id, Some(membership_log_id))
            .is_ok()
        },
