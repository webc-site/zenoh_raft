...
use crate::{
  NodeId,
  vote::{LeaderIdCompare, RaftLeaderId, RaftTerm},
};
...
mod tests {
  ...
  #[test]
  fn test_committed_leader_id_bitcode() -> anyhow::Result<()> {
    use super::CommittedLeaderId;

    let c = CommittedLeaderId::<u64>::new(5);
    let bytes = bitcode::encode(&c);
    let c2: CommittedLeaderId<u64> = bitcode::decode(&bytes)?;
    assert_eq!(CommittedLeaderId::new(5), c2);

    let lid = LeaderId::<u64, u64>::new(5, 3);
    let bytes = bitcode::encode(&lid);
    let decoded: LeaderId<u64, u64> = bitcode::decode(&bytes)?;
  ...
  fn test_std_leader_id_partial_order() -> anyhow::Result<()> {
    ...
    // Incomparable
    assert_eq!(lid(2, 2).partial_cmp(&lid(2, 3)), None);
    assert_ne!(lid(2, 2), lid(2, 3));
  ...
  fn test_leader_id_vs_committed_partial_order() -> anyhow::Result<()> {
    ...
    assert_ne!(lid(2, 2), clid(2));

    // Same term: CommittedLeaderId > LeaderId
    assert_eq!(lid(2, 2).partial_cmp(&clid(2)), Some(Less));
