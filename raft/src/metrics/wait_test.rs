...
use crate::{
  Membership, RaftMetrics, RaftTypeConfig, Vote,
  core::ServerState,
  engine::testing::{UTConfig, log_id},
  log_id::LogIdOptionExt,
  metrics::{SerdeInstant, Wait, WaitError},
  type_config::{
    TypeConfigExt,
    alias::{NodeIdOf, StoredMembershipOf, VoteOf, WatchSenderOf},
  },
...
fn test_wait() {
  UTConfig::<()>::run(async {
    ...
    log::info!("--- wait for snapshot, only index matches");
    {
      let (init, w, tx) = init_wait_test::<UTConfig>();
...
fn test_wait_leader_with_quorum_acked() {
  UTConfig::<()>::run(async {
    ...
    let got = w
      .leader_with_quorum_acked(Some(second_acked), "newer quorum ack")
      .await?;
    assert_eq!(
  ...
  C: RaftTypeConfig<NodeId = u64>,
{
  let init = RaftMetrics {
    ...
    committed_membership_config: Arc::new(StoredMembershipOf::<C>::new(
      None,
      Membership::default(),
    )),
    heartbeat: None,
