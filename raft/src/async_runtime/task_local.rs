//! Task-local storage for async runtimes.
//!
//! This module is adapted from [tokio::task::LocalKey](https://docs.rs/tokio/latest/tokio/task/struct.LocalKey.html)
//! (tokio v1.44, MIT licensed) to decouple from the tokio runtime.
//!
//! Provides [`LocalKey`] and the [`task_local!`](crate::task_local!) macro for declaring
//! task-scoped variables that are swapped in/out via `scope()` or `sync_scope()`.
...
/// Publicity and attributes for each static are preserved.
///
/// # Examples
///
/// ```ignore
...
macro_rules! __task_local_inner {
    ($(#[$attr:meta])* $vis:vis $name:ident, $t:ty) => {
        $(#[$attr])*
        $vis static $name: $crate::async_runtime::task_local::LocalKey<$t> = {
            thread_local! {
                static __KEY: ::std::cell::RefCell<Option<$t>> = const { ::std::cell::RefCell::new(None) };
            }

            $crate::async_runtime::task_local::LocalKey { inner: __KEY }
        };
    };
}
...
///
/// This type is generated by the [`task_local!`](crate::task_local!) macro.
///
    ...
    T: 'static,
{
    ...
    ///
    /// Returns `Some(T)` if the task local value exists, `None` if it has
    /// already been taken.
...
impl From<thread::AccessError> for ScopeInnerErr {
    fn from(_: thread::AccessError) -> Self {
        Self::AccessError
    }
}
