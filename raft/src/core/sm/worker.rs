...
use crate::{
  RaftLogReader, RaftSnapshotBuilder, RaftTypeConfig, StorageError,
  core::{
    ApplyResult,
    notification::Notification,
    sm::{Command, CommandResult, Response, handle::Handle},
  },
  entry::RaftEntry,
  errors::StorageIOResult,
  raft::responder::core_responder::CoreResponder,
  storage::{RaftStateMachine, entry_responder::EntryResponderBuilder},
  type_config::{
    TypeConfigExt,
    alias::{JoinHandleOf, LogIdOf, MpscReceiverOf, MpscSenderOf, OneshotSenderOf, SnapshotOf},
  },
};
  ...
  LR: RaftLogReader<C>,
{
  ...
  pub(crate) fn spawn(
    ...
    let (cmd_tx, cmd_rx) = C::mpsc::<Command<C, SM>>(state_machine_channel_size);
  ...
  fn do_spawn(mut self) -> JoinHandleOf<C, ()> {
    let fu = async move {
      ...
      if let Err(err) = res {
        log::error!("{} while execute state machine command", err,);
  ...
  async fn worker_loop(&mut self) -> Result<(), StorageError<C>> {
    loop {
      ...
      let cmd = match cmd {
        None => {
          log::info!("{}: rx closed, state machine worker quit", func_name!());
          return Ok(());
        }
        Some(x) => x,
      };

      log::debug!("RAFT_event id={:<2}  input: {:?}", self.id, cmd);

      match cmd {
        Command::BuildSnapshot => {
          log::info!("{}: build snapshot", func_name!());
        ...
        Command::GetSnapshot { tx } => {
          log::info!("{}: get snapshot", func_name!());
        ...
        Command::InstallFullSnapshot {
          ...
          log::info!("{}: install complete snapshot", func_name!());
          ...
          log::info!("Done install complete snapshot, meta: {}", meta);

          self
            .send_response(Response::InstallSnapshot((io_id, Some(meta))))
            .await;
        }
        Command::Apply {
          first,
          ...
          self.send_response(Response::Apply(resp)).await;
        }
        Command::ExternalFunc { func } => {
          log::debug!("{}: run user defined ExternalFunc", func_name!());
  ...
  async fn send_response(&mut self, resp: Response<C>) {
    let res = CommandResult::new(Ok(resp));
  ...
  async fn apply(
    ...
    let strm = strm.map_ok(move |entry| {
      ...
      #[cfg(debug_assertions)]
      last_apply.store(log_index, Relaxed);

      log::debug!("Applying entry to state machine: {}", item);
    ...
    self
      ...
      .apply(Box::pin(strm))
      .await
      .sto_apply(last.clone())?;

    #[cfg(debug_assertions)]
    {
      assert_eq!(end - 1, got_last_index.load(Relaxed));
    }
  ...
  async fn build_snapshot(&mut self, resp_tx: MpscSenderOf<C, Notification<C>>) {
    ...
    let Some(mut builder) = builder else {
      log::info!(
        "{}: snapshot building is refused by state machine",
        func_name!()
      );
      let res = CommandResult::new(Ok(Response::<C>::BuildSnapshotDone(None)));
      resp_tx.send(Notification::<C>::sm(res)).await.ok();
      return;
    };

    drop(C::spawn(async move {
      let res = builder.build_snapshot().await.sto_write_snapshot(None);
      let res = res.map(|snap| Response::<C>::BuildSnapshotDone(Some(snap.meta)));
      let cmd_res = CommandResult::new(res);
      resp_tx.send(Notification::<C>::sm(cmd_res)).await.ok();
    }));
    log::info!(
      "{}: returning; spawned building snapshot task",
      func_name!()
    );
  }
