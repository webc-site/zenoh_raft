...
use crate::{
  RaftTypeConfig,
  async_runtime::deterministic_rng::DeterministicRng,
  core::notification::Notification,
  type_config::{
    TypeConfigExt,
    alias::{JoinHandleOf, MpscSenderOf, OneshotReceiverOf, OneshotSenderOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  pub(crate) fn spawn(
    ...
    let join_handle = C::spawn(this.tick_loop(shutdown_rx));
  ...
  fn sample_first_wait(period: Duration) -> Duration {
    ...
    use rand::RngExt;
    DeterministicRng::thread_rng().random_range(period..period * 2)
  }

  pub(crate) async fn tick_loop(self, cancel_rx: OneshotReceiverOf<C, ()>) {
    let mut i = 0;

    let mut cancel = pin!(cancel_rx);
    ...
    loop {
      let sleep_fut = pin!(C::sleep_until(at));
      let cancel_fut = cancel.as_mut();

      match future::select(cancel_fut, sleep_fut).await {
        Either::Left((_canceled, _)) => {
          log::info!("TickLoop received cancel signal, quit");
          return;
        }
        Either::Right((..)) => {
          // sleep done
      ...
      let send_res = self.tx.send(Notification::<C>::Tick { i }).await;
      if let Err(_e) = send_res {
        log::info!("Stopping tick_loop(), main loop terminated");
        break;
      } else {
        log::debug!("Tick sent: {i}")
      }
    }
  }
}
  ...
  pub(crate) fn shutdown(&self) -> Option<JoinHandleOf<C, ()>> {
    {
      ...
      if let Some(shutdown) = shutdown {
        let send_res = ();
        shutdown.send(());
        log::info!("Timer shutdown signal sent: {send_res:?}");
      } else {
        log::warn!("Double call to Raft::shutdown()");
      }
    }
...
mod tests {
  ...
  use crate::{
    ...
    async_runtime::deterministic_rng::DeterministicRng,
    core::{Tick, notification::Notification},
    impls::{InlineBatch, OneshotResponder, Vote, leader_id_adv::LeaderId},
    type_config::{TypeConfigExt, alias::MpscReceiverOf},
    vote::RaftLeaderId,
  };
    ...
    T: Send,
  {
    Runtime::new()
      .unwrap()
      .block_on(DeterministicRng::scope(seed, future))
  }
  ...
  fn test_sample_first_wait_preserves_duration_precision() {
    ...
    for period in periods {
      ...
      let expected = run_seeded(SEED, async {
        (0..SAMPLES)
          .map(|_| DeterministicRng::thread_rng().random_range(period..period * 2))
          .collect::<Vec<_>>()
      });
  ...
  fn test_sample_first_wait_is_uniformly_distributed() {
    ...
    assert!(
      counts
        .iter()
        .all(|count| count.abs_diff(SAMPLES_PER_BUCKET) <= MAX_DEVIATION),
  ...
  fn test_shutdown_interrupts_first_wait() {
    TickUTConfig::run(async {
      ...
      assert!(
        ...
        "no tick should precede the first wait"
      );
    });
  }
  ...
  fn test_only_first_wait_is_randomized() {
    ...
    let margin = Duration::from_millis(60);
    let first_wait = run_seeded(SEED, async {
      Tick::<SeededTickConfig>::sample_first_wait(period)
    });
    assert!(
    ...
    run_seeded(SEED, async {
      ...
      let early_first =
        SeededTickConfig::timeout(first_wait - margin, recv_tick::<SeededTickConfig>(&mut rx))
          .await;
      assert!(
        early_first.is_err(),
        "the first tick must include the sampled phase offset"
      );
      let first = SeededTickConfig::timeout(margin * 6, recv_tick::<SeededTickConfig>(&mut rx))
        .await
        .unwrap();
      ...
      assert!(
        early_second.is_err(),
        "the second tick must wait for a full period"
      );
      let second = SeededTickConfig::timeout(margin * 6, recv_tick::<SeededTickConfig>(&mut rx))
        .await
        .unwrap();
      ...
      assert!(
        rx.recv().await.is_none(),
        "the channel must close after shutdown"
      );
    });
  }
  ...
  fn test_a_stall_skips_missed_periods_and_keeps_the_phase() {
    ...
    run_seeded(SEED, async {
      ...
      let early =
        SeededTickConfig::timeout(PERIOD / 4, recv_tick::<SeededTickConfig>(&mut rx)).await;
      assert!(
        early.is_err(),
        "the missed periods must be skipped, not emitted"
      );
