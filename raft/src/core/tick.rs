...
use crate::async_runtime::deterministic_rng::DeterministicRng;
...
mod tests {
    ...
    fn test_only_first_wait_is_randomized() {
        ...
        run_seeded(SEED, async {
            ...
            let early_first = SeededTickConfig::timeout(
                first_wait - margin,
                recv_tick::<SeededTickConfig>(&mut rx),
            )
            .await;
            assert!(
                early_first.is_err(),
            ...
            let first =
                SeededTickConfig::timeout(margin * 6, recv_tick::<SeededTickConfig>(&mut rx))
                    .await
                    .unwrap();

            let early_second =
                SeededTickConfig::timeout(period - margin, recv_tick::<SeededTickConfig>(&mut rx))
                    .await;
            assert!(
                early_second.is_err(),
    ...
    fn test_a_zero_period_keeps_ticking() {
        TickUTConfig::run(async {
            ...
            for expected in 1..=3 {
                let i = TickUTConfig::timeout(
                    Duration::from_secs(1),
                    recv_tick::<TickUTConfig>(&mut rx),
                )
                .await
    ...
    fn test_a_stall_skips_missed_periods_and_keeps_the_phase() {
        ...
        run_seeded(SEED, async {
            ...
            let fourth =
                SeededTickConfig::timeout(PERIOD / 2, recv_tick::<SeededTickConfig>(&mut rx))
                    .await
