use crate::{
  ...
  type_config::{
    TypeConfigExt,
    alias::{InstantOf, JoinHandleOf, MpscSenderOf, WatchReceiverOf, WatchSenderOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  ...
  async fn run(
    ...
    loop {
      ...
      let send_result = notification_tx
        .send(Notification::<C>::PendingReadDeadlineReached)
        .await;
...
mod tests {
  ...
  fn deadline_notifier_uses_latest_deadline_and_rearms() {
    C::run(async {
      ...
      assert!(matches!(
        notification,
        Notification::PendingReadDeadlineReached
      ));
