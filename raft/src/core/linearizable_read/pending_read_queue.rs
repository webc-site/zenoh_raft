...
use super::{
  pending_read::PendingRead, pending_read_deadline_key::PendingReadDeadlineKey,
  pending_read_key::PendingReadKey,
};
use crate::{
  RaftTypeConfig,
  errors::LinearizableReadError,
  ...
  C: RaftTypeConfig,
{
  pub(crate) fn push(&mut self, min_quorum_acked_at: InstantOf<C>, pending_read: PendingRead<C>) {
    ...
    self.next_sequence = sequence
      .checked_add(1)
      .expect("pending read sequence overflow");
  ...
  pub(crate) fn drain_expired(
    ...
    while let Some((deadline_key, key)) = self.deadlines.first_key_value() {
      ...
      let pending_read = self
        .reads
        .remove(&key)
        .expect("a deadline entry must have its read");
      let err = make_error(key.min_quorum_acked_at);
      pending_read.response_tx.send(Err(err));
    }
    self.debug_assert_indexes_agree();
  }
  ...
  pub(crate) fn drain_satisfied(
    &mut self,
    quorum_acked_at: InstantOf<C>,
    ...
    while let Some((key, _)) = self.reads.first_key_value() {
      ...
      self
        .deadlines
        .remove(&deadline_key)
        .expect("a read must have its deadline entry");

      let linearizer = pending_read.linearizer.with_applied(applied.clone());
      pending_read.response_tx.send(Ok(linearizer));
    }
    self.debug_assert_indexes_agree();
  }
  ...
  pub(crate) fn drain_all_with_error(&mut self, err: LinearizableReadError<C>) {
    ...
    let pending_reads = mem::take(&mut self.reads);
    for pending_read in pending_reads.into_values() {
      pending_read.response_tx.send(Err(err.clone()));
    }
  }
