...
use crate::{
  Config, RaftTypeConfig, StepDownPolicy,
  async_runtime::MpscWeakSender,
  core::{
    ServerState,
    raft_msg::{RaftMsg, external_command::ExternalCommand},
  },
  metrics::{RaftMetrics, RaftServerMetrics},
  type_config::{
    TypeConfigExt,
    alias::{MpscWeakSenderOf, WatchReceiverOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) fn spawn(
    ...
    let watch_fut = this.watch_loop();
    drop(C::spawn(watch_fut));
  }
  ...
  async fn watch_loop(mut self) {
    loop {
      ...
      if recv_res.is_err() {
        log::info!("RaftCore terminated, quit step_down_watcher");
        return;
      }
    }
  }
  ...
  async fn try_step_down(&self) {
    ...
    if let Some(to) = target {
      log::info!("removed Leader steps down: transfer leadership to {}", to);
      self
        .send(ExternalCommand::TriggerTransferLeader { to })
        .await;
    }
  ...
  async fn send(&self, cmd: ExternalCommand<C>) {
    let Some(tx_api) = MpscWeakSender::<RaftMsg<C>>::upgrade(&self.tx_api) else {
      return;
    };
...
mod tests {
  ...
  use super::{is_removed_leader, transfer_target};
  use crate::{
    Membership,
    core::ServerState,
    engine::testing::{UTConfig, log_id},
    metrics::{RaftMetrics, RaftServerMetrics},
    type_config::alias::StoredMembershipOf,
  };
  ...
  fn test_is_removed_leader_learner_leader() {
    let m = server_metrics(Membership::new_with_defaults(
      vec![btreeset! {2,3}],
      btreeset! {1},
    ));
  ...
  fn test_is_removed_leader_uncommitted_membership() {
    ...
    m.committed_membership_config =
      stored(1, Membership::new_with_defaults(vec![btreeset! {1,2}], []));
