...
use crate::{
  ...
  type_config::{
    TypeConfigExt,
    alias::{InstantOf, MpscReceiverOf},
  },
};
  ...
  C: RaftTypeConfig,
{
  ...
  /// - linger
  pub(crate) fn new(
    receiver: MpscReceiverOf<C, RaftMsg<C>>,
    capacity: u64,
    linger: Duration,
  ) -> Self {
    Self {
  ...
  async fn inner_recv(&mut self) -> Result<RaftMsg<C>, Fatal<C>> {
    let Some(msg) = self.inner.recv().await else {
      log::info!("all rx_api senders are dropped");
  ...
  async fn inner_recv_timeout_at(
    ...
    match C::timeout_at(deadline, self.inner.recv()).await {
      Ok(Some(value)) => Ok(Some(value)),
      Ok(None) => {
        log::debug!("rx_api is disconnected, quit");
        Err(Fatal::Stopped)
      }
      Err(_) => {
        log::debug!("all RaftMsg are processed, wait for more");
        Ok(None)
      }
    }
  }
...
mod tests {
  ...
  use crate::{
    ...
    type_config::{
      TypeConfigExt,
      alias::{BatchOf, CommittedLeaderIdOf, PayloadOf},
    },
  };
  ...
  fn test_ensure_buffered_returns_immediately_if_already_buffered() {
    C::run(async {
      ...
      let _ = receiver.try_recv().await.unwrap().unwrap();
  ...
  fn test_try_recv_batch_up_to_max_capacity() {
    ...
    C::run(async {
      ...
      let mut receiver: BatchRaftMsgReceiver<C> =
        BatchRaftMsgReceiver::new(rx, BATCH_CAPACITY, Duration::ZERO);
  ...
  fn test_try_recv_flush_batch_after_linger() {
    ...
    C::run(async {
      ...
      let mut receiver: BatchRaftMsgReceiver<C> =
        BatchRaftMsgReceiver::new(rx, BATCH_CAPACITY, LINGER);
