...
use crate::{
  RaftTypeConfig,
  engine::{Respond, respond_command::PendingRespond},
  raft_state::{IOId, io_state::IOState},
  ...
  C: RaftTypeConfig,
{
  ...
  pub(crate) fn drain_satisfied<'a>(
    &'a mut self,
...
pub(crate) fn pop_if_satisfied<C, V>(
  queue: &mut VecDeque<PendingRespond<C, V>>,
  actual: &V,
) -> Option<Respond<C>>
where
  C: RaftTypeConfig,
...
///
/// Iterates through all phases (LogIO, Snapshot) and yields responds
      ...
      match self.phase {
        ...
        DrainPhase::Snapshot => {
          if let Some(snapshot) = self.io_state.snapshot.flushed()
            && let Some(respond) =
              pop_if_satisfied(&mut self.pending_responds.on_snapshot, snapshot)
          {
            return Some((self.phase, respond));
          }
...
mod tests {
  ...
  use crate::{
    ...
    engine::{
      Respond,
      respond_command::PendingRespond,
      testing::{UTConfig, log_id},
    },
    raft::VoteResponse,
    raft_state::{
      IOId,
      io_state::{IOState, io_progress::IOProgress},
    },
  ...
  fn test_drain_satisfied_log_io() {
    ...
    pending
      .on_log_io
      .push_back(PendingRespond::new(io_id, new_respond()));
  ...
  fn test_drain_satisfied_snapshot() {
    ...
    pending
      .on_snapshot
      .push_back(PendingRespond::new(log_id(1, 1, 3), new_respond()));
  ...
  fn test_drain_satisfied_multiple_phases() {
    ...
    assert_eq!(collected[0].0, DrainPhase::LogIO);
    assert_eq!(collected[1].0, DrainPhase::Snapshot);
  }
  ...
  fn test_drain_satisfied_multiple_items_per_phase() {
    ...
    pending
      .on_log_io
      .push_back(PendingRespond::new(io_id1, new_respond()));
    pending
      .on_log_io
      .push_back(PendingRespond::new(io_id2, new_respond()));
    pending
      .on_log_io
      .push_back(PendingRespond::new(io_id3, new_respond()));
    ...
    pending
      .on_snapshot
      .push_back(PendingRespond::new(log_id(1, 1, 10), new_respond()));
    ...
    assert_eq!(collected[2].0, DrainPhase::Snapshot);
  ...
  fn test_drain_phase_display() {
    let p1 = DrainPhase::LogIO;
    let p2 = DrainPhase::Snapshot;
    let p3 = DrainPhase::Done;
    assert_eq!(format!("{p1}"), "log_io");
    assert_eq!(format!("{p2}"), "snapshot");
    assert_eq!(format!("{p3}"), "done");
  }
}
