...
fn test_handle_vote_req_rejected_by_leader_lease() -> anyhow::Result<()> {
  ...
  assert_eq!(
    VoteResponse::new(Vote::new_committed(2, 1), None, false),
    resp
  );
...
fn test_handle_vote_req_leadership_transfer_overrides_leader_lease() -> anyhow::Result<()> {
  ...
  assert!(
    ...
    "candidate yields to the new candidate"
  );
...
fn test_handle_vote_req_leadership_transfer_overrides_quorum_ack_lease() -> anyhow::Result<()> {
  ...
  assert!(
    eng.leader.is_none(),
    "voting for another node steps the Leader down"
  );
...
fn test_handle_vote_req_reject_smaller_last_log_id() -> anyhow::Result<()> {
  ...
  assert_eq!(
    VoteResponse::new(Vote::new(2, 1), Some(log_id(2, 1, 3)), false),
    resp
  );
...
fn test_handle_vote_req_granted_equal_vote_and_last_log_id() -> anyhow::Result<()> {
  ...
  assert_eq!(
    VoteResponse::new(Vote::new(2, 1), Some(log_id(2, 1, 3)), true),
    resp
  );
...
fn test_handle_vote_req_granted_greater_vote() -> anyhow::Result<()> {
  ...
  assert_eq!(
    VoteResponse::new(Vote::new(3, 1), Some(log_id(2, 1, 3)), true),
    resp
  );
