...
fn test_handle_pre_vote_resp_quorum_starts_real_election() -> anyhow::Result<()> {
  ...
  eng.handle_pre_vote_resp(
    2,
    VoteResponse::new(Vote::new(0, 0), Some(log_id(1, 1, 1)), true),
  );
...
fn test_winning_election_cancels_overlapping_pre_vote() -> anyhow::Result<()> {
  ...
  assert_eq!(
    vec![
      Command::SendPreVote {
        vote_req: VoteRequest::new_pre_vote(Vote::new(1, 1), Some(log_id(1, 1, 1))),
      },
      Command::SaveVote {
        vote: Vote::new(1, 1)
      },
      ...
      Command::SendPreVote {
        vote_req: VoteRequest::new_pre_vote(Vote::new(2, 1), Some(log_id(1, 1, 1))),
      },
    ],
    eng.output.take_commands()
  );
  ...
  eng.handle_vote_resp(
    2,
    VoteResponse::new(Vote::new(1, 1), Some(log_id(1, 1, 1)), true),
  );
...
fn test_handle_pre_vote_resp_reject_keeps_waiting() -> anyhow::Result<()> {
  ...
  eng.handle_pre_vote_resp(
    2,
    VoteResponse::new(Vote::new(0, 0), Some(log_id(1, 1, 1)), false),
  );
...
fn test_handle_pre_vote_resp_reject_adopts_higher_vote() -> anyhow::Result<()> {
  ...
  eng.handle_pre_vote_resp(
    2,
    VoteResponse::new(Vote::new(10, 2), Some(log_id(1, 1, 1)), false),
  );
...
fn test_handle_pre_vote_resp_reject_equal_vote_unchanged() -> anyhow::Result<()> {
  ...
  eng.handle_pre_vote_resp(
    2,
    VoteResponse::new(Vote::new(5, 1), Some(log_id(1, 1, 1)), false),
  );

  assert!(
    eng.pre_candidate_ref().is_some(),
    "pre-vote round still in flight"
  );
