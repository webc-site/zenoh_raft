...
use crate::{
  LogId, RaftLogReader, RaftTypeConfig, StorageError,
  engine::leader_log_ids::LeaderLogIds,
  log_id::{
    option_raft_log_id_ext::OptionRaftLogIdExt, raft_log_id::RaftLogId,
    raft_log_id_ext::RaftLogIdExt, ref_log_id::RefLogId,
  },
  storage::RaftLogReaderExt,
  vote::{RaftCommittedLeaderId, RaftLeaderId},
};
  ...
  CLID: RaftCommittedLeaderId,
{
  ...
  fn push_key_log_id(res: &mut Vec<LogId<CLID>>, log_id: LogId<CLID>) {
  ...
  pub(crate) async fn get_key_log_ids<C, LR>(
    range: RangeInclusive<LogId<CLID>>,
    sto: &mut LR,
  ) -> Result<Vec<LogId<CLID>>, StorageError<C>>
  where
    C: RaftTypeConfig<LeaderId: RaftLeaderId<Committed = CLID>>,
    LR: RaftLogReader<C> + ?Sized,
  {
    let first = range.start().clone();
    let last = range.end().clone();

    let mut res: Vec<LogId<CLID>> = vec![];
  ...
  pub fn new(
    purged: Option<LogId<CLID>>,
    key_log_ids: impl IntoIterator<Item = LogId<CLID>>,
  ) -> Self {
    Self {
      purged,
  ...
  pub(crate) fn append(&mut self, new_log_id: LogId<CLID>) {
  ...
  pub(crate) fn truncate(&mut self, at: u64) {
    ...
    if at > end_after_truncate {
      ...
      self
        .key_log_ids
        .push(LogId::<CLID>::new(truncated_leader, at - 1));
    } else {
  ...
  pub(crate) fn purge(&mut self, upto: &LogId<CLID>) {
    ...
    let res = self
      .key_log_ids
      .binary_search_by(|log_id| log_id.index().cmp(&upto_index));
  ...
  pub(crate) fn ref_at(&self, index: u64) -> Option<RefLogId<'_, CLID>> {
    // Handle purged range
    ...
    Some(RefLogId::new(
      self.key_log_ids[i].committed_leader_id(),
      index,
    ))
  }
  ...
  pub(crate) fn first(&self) -> Option<RefLogId<'_, CLID>> {
  ...
  pub(crate) fn last_ref(&self) -> Option<RefLogId<'_, CLID>> {
    self.last().map(|x| x.to_ref())
  }

  pub(crate) fn key_log_ids(&self) -> &[LogId<CLID>] {
    &self.key_log_ids
  }
  ...
  pub(crate) fn by_last_leader(&self) -> Option<LeaderLogIds<CLID>> {
    let last = self.last()?;
