...
use crate::{
  ...
  config::Config,
  engine::{
    Command, Condition, command_scheduler::CommandScheduler, pending_responds::PendingResponds,
  ...
  SM: RaftStateMachine<C>,
{
  ...
  pub(crate) fn postpone_command(&mut self, cmd: Command<C, SM>) -> Result<(), &'static str> {
    log::debug!("postpone command: {:?}", cmd);
      ...
      Command::Respond { when, resp } => {
        ...
        match when {
          ...
          Some(Condition::IOFlushed { io_id }) => {
            pending_responds
              .on_log_io
              .push_back(PendingRespond::new(io_id, resp));
          }
          Some(Condition::Snapshot { log_id }) => {
            pending_responds
              .on_snapshot
              .push_back(PendingRespond::new(log_id, resp));
          }
        }
        Ok(())
      }
