...
use crate::{
  OptionalSend, RaftState, RaftTypeConfig,
  display_ext::display_instant::DisplayInstantExt,
  engine::{
    Command, Condition, EngineConfig, EngineOutput, Respond, ValueSender,
    handler::{
      leader_handler::LeaderHandler, replication_handler::ReplicationHandler,
      server_state_handler::ServerStateHandler,
    },
  },
  entry::RaftPayload,
  errors::RejectVote,
  proposer::{CandidateState, LeaderState},
  raft_state::{IOId, LogStateReader},
  storage::RaftStateMachine,
  type_config::{
    TypeConfigExt,
    alias::{OneshotSenderOf, VoteOf},
  },
  vote::{RaftLeaderId, RaftVote, raft_vote::RaftVoteExt},
};
  ...
  SM: RaftStateMachine<C>,
{
  ...
  where
    T: Debug + Eq + OptionalSend + 'static,
  ...
  pub(crate) fn update_vote(&mut self, vote: &VoteOf<C>) -> Result<(), RejectVote<C>> {
    ...
    log::debug!("vote is changing to: {vote}");
    ...
    if vote.as_ref_vote() > self.state.vote_ref().as_ref_vote() {
      ...
      self
        .output
        .push_command(Command::SaveVote { vote: vote.clone() });
    } else {
      let now = C::now();
  ...
  pub(crate) fn become_leader(&mut self) {
    ...
    if let Some(l) = self.leader.as_mut() {
      log::debug!("leading vote: {}", l.committed_vote,);
    ...
    *self.leader = Some(Box::new(leader));
    ...
    self
      .state
      .accept_log_io(IOId::new_log_io(leader_vote.clone(), last_log_id.clone()));
  ...
  pub(crate) fn become_following(&mut self) {
    ...
    let had_leading_state =
      self.leader.is_some() || self.candidate.is_some() || self.pre_candidate.is_some();
