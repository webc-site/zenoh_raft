...
where
    F: FnOnce(&mut ProgressEntry<UTConfig>),
{
...
fn ordered_matching(eng: &Engine<UTConfig>) -> Vec<(u64, Option<LogIdOf<UTConfig>>)> {
    ...
    leader
        .progress
        .collect_mapped(|entry| (entry.id, entry.matching))
}
...
fn test_update_conflicting_response_identity() -> anyhow::Result<()> {
    ...
    log::info!("target=2 install the first payload request");
    {
        update_entry(&mut eng, 2, |entry| {
            entry.matching = Some(log_id(2, 1, 3));
            entry.data.searching_end = 10;
            entry.data.inflight =
                Inflight::logs(Some(log_id(2, 1, 7)), Some(log_id(2, 1, 9)), first_id);
        });
    }

    log::info!("target=2 inflight_id=7 apply the matching conflict response");
    {
        ...
        eng.replication_handler()
            .update_conflicting(2, log_id(2, 1, 7), Some(first_id));

        let actual = entry(&eng, 2);
        assert_eq!(expected, actual);
    }

    log::info!("target=2 inflight_id=8 replace the completed payload request");
    {
        update_entry(&mut eng, 2, |entry| {
            entry.data.searching_end = 10;
            entry.data.inflight =
                Inflight::logs(Some(log_id(2, 1, 3)), Some(log_id(2, 1, 9)), second_id);
        });
    }

    log::info!("target=2 inflight_id=7 ignore a stale conflict from the first request");
    {
    ...
    log::info!("target=2 apply a heartbeat conflict without clearing payload state");
    {
        let mut expected = entry(&eng, 2);
        expected.data.searching_end = 6;

        eng.replication_handler()
            .update_conflicting(2, log_id(2, 1, 6), None);

        let actual = entry(&eng, 2);
        assert_eq!(expected, actual);
    }

    log::info!("target=99 ignore a conflict for a removed target");
    {
        let expected_entries = entries(&eng);
        let expected_quorum = quorum_accepted(&eng);

        eng.replication_handler()
            .update_conflicting(99, log_id(2, 1, 0), Some(first_id));
...
fn test_update_conflicting_reversion() -> anyhow::Result<()> {
    ...
    log::info!("establish ordered voter progress and a quorum watermark");
    {
        update_entry(&mut eng, 1, |entry| {
            entry.matching = Some(log_id(2, 1, 8));
            entry.data.searching_end = 12;
            entry.data.inflight =
                Inflight::logs(Some(log_id(2, 1, 8)), Some(log_id(2, 1, 11)), first_id);
        });
        update_entry(&mut eng, 2, |entry| {
    ...
    log::info!("target=1 conflict=8 apply globally enabled log reversion");
    {
        ...
        eng.replication_handler()
            .update_conflicting(1, log_id(2, 1, 8), Some(first_id));
    ...
    log::info!("target=3 conflict=6 consume one-shot log reversion permission");
    {
        eng.config.allow_log_reversion = false;
        update_entry(&mut eng, 3, |entry| {
            entry.data.searching_end = 10;
            entry.data.inflight =
                Inflight::logs(Some(log_id(2, 1, 6)), Some(log_id(2, 1, 9)), third_id);
        });
        ...
        eng.replication_handler()
            .update_conflicting(3, log_id(2, 1, 6), Some(third_id));
