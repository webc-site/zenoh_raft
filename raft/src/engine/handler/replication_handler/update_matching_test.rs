...
fn test_update_matching_no_leader() -> anyhow::Result<()> {
    ...
    let res = panic::catch_unwind(move || {
        ...
        eng.replication_handler()
            .update_matching(3, Some(log_id(1, 1, 2)), None);
    });
    log::info!("res: {:?}", res);
    assert!(res.is_err());
...
fn test_update_matching() -> anyhow::Result<()> {
    ...
    let mut set_inflight = |id, prev| {
        ...
        assert_eq!(
            ...
            rh.leader
                .progress
                .update_data_with(&id, |data| data.inflight = inflight)
                .map(|data| &data.inflight)
        );
    };
    ...
    // progress: None, (2,1), (2,3); committed: (2,1)
    {
        ...
        assert_eq!(
            Some(&log_id(2, 1, 1)),
            rh.state.io_state.apply_progress.accepted()
        );
