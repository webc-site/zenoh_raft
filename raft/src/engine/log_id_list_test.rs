...
use crate::{
  ...
  engine::{
    ...
    testing::{UTConfig, UtClid, log_id},
  },
  entry::RaftEntry,
  log_id::raft_log_id_ext::RaftLogIdExt,
  type_config::{
    TypeConfigExt,
...
impl RaftLogReader<UTConfig> for MockLogReader {
  async fn try_get_log_entries<RNG>(
    &mut self,
    range: RNG,
  ) -> Result<Vec<<UTConfig as crate::RaftTypeConfig>::Entry>, Error>
  where
    RNG: RangeBounds<u64> + Clone + Debug + crate::OptionalSend,
  {
    use std::ops::Bound;
    ...
    for i in start..end {
      if let Some(log_id) = self.logs.get(&i) {
        ...
        entries.push(EntryOf::<UTConfig>::new_blank(*log_id));
      }
    }
    Ok(entries)
  }
  ...
  async fn read_vote(
    &mut self,
  ) -> Result<Option<<UTConfig as crate::RaftTypeConfig>::Vote>, Error> {
    Ok(None)
  }
}
...
fn test_log_id_list_purge() -> anyhow::Result<()> {
  ...
  let cases: Vec<(_, Option<_>, Vec<_>)> = vec![
    ...
    (
      log_id(2, 1, 2),
      Some(log_id(2, 1, 2)),
      vec![log_id(3, 1, 5), log_id(6, 1, 8), log_id(9, 1, 11)],
    ),
    ...
    (
      log_id(3, 1, 5),
      Some(log_id(3, 1, 5)),
      vec![log_id(6, 1, 8), log_id(9, 1, 11)],
    ),
...
fn test_log_id_list_by_last_leader() -> anyhow::Result<()> {
  ...
  assert_eq!(
    Some(LeaderLogIds::new(
      *log_id(1, 1, 0).committed_leader_id(),
      0,
      1
    )),
    ids.by_last_leader()
  ...
  assert_eq!(
    Some(LeaderLogIds::new(
      *log_id(3, 1, 0).committed_leader_id(),
      2,
      3
    )),
    ids.by_last_leader()
  ...
  assert_eq!(
    Some(LeaderLogIds::new(
      *log_id(7, 1, 0).committed_leader_id(),
      3,
      10
    )),
    ids.by_last_leader()
...
fn test_log_id_list_by_last_leader_edge_cases() -> anyhow::Result<()> {
  ...
  assert_eq!(
    Some(LeaderLogIds::new(
      *log_id(2, 1, 0).committed_leader_id(),
      3,
      5
    )),
    ids.by_last_leader()
...
fn test_get_key_log_ids_single_log() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(1, 1, 0), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_all_same_leader() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(1, 1, 2), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_two_adjacent_leaders() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(2, 1, 1), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_case_aac() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(2, 1, 2), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_case_acc() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(2, 1, 3), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_case_abc() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(3, 1, 2), &mut reader)
      .await
      .unwrap();

    assert_eq!(
      vec![log_id(1, 1, 0), log_id(2, 1, 1), log_id(3, 1, 2)],
      result
    );
  });
}
...
fn test_get_key_log_ids_many_same_leader() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(2, 1, 4), &mut reader)
      .await
      .unwrap();
...
fn test_get_key_log_ids_three_leaders() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(3, 1, 5), &mut reader)
      .await
      .unwrap();

    assert_eq!(
      vec![log_id(1, 1, 0), log_id(2, 1, 3), log_id(3, 1, 5)],
      result
    );
  });
}
...
fn test_get_key_log_ids_long_same_leader() {
  UTConfig::<()>::run(async {
    ...
    let result = LogIdList::get_key_log_ids(log_id(1, 1, 0)..=log_id(1, 1, 9), &mut reader)
      .await
      .unwrap();
