use crate::{
  ...
  batch::Batch,
  config::Config,
  engine::{Command, engine_output::EngineOutput},
    ...
    while let Some(next) = commands.pop_front() {
      match next {
        Command::AppendEntries {
          ...
          && entries.len() as u64 + next_entries.len() as u64 <= max_entries =>
        {
          n += 1;
...
mod tests {
  ...
  use crate::{
    ...
    engine::{
      ...
      testing::{UTConfig, UTLeaderId},
    },
    impls::Vote,
    testing::blank_ent,
    vote::{committed::CommittedVote, raft_vote::RaftVoteExt},
  };
  ...
  fn test_merge_stops_at_max_entries() {
    ...
    output.push_command(Command::AppendEntries {
      committed_vote: committed_vote(1, 0),
      entries: Batch::of([
        blank_ent::<UTConfig>(1, 0, 1),
        blank_ent::<UTConfig>(1, 0, 2),
      ]),
    });
    output.push_command(Command::AppendEntries {
      committed_vote: committed_vote(1, 0),
