...
use crate::{
  ...
  errors::RPCError,
  network::{NetAppend, RPCOption},
  raft::{AppendEntriesRequest, StreamAppendResult},
};

pub type AppendResponseStream<'s, C> = BoxStream<'s, Result<StreamAppendResult<C>, RPCError<C>>>;
pub type StreamAppendFuture<'s, C> =
  BoxFuture<'s, Result<AppendResponseStream<'s, C>, RPCError<C>>>;
...
/// 1. **From RaftNetwork**: Blanket impl delegates to `RaftNetwork::stream_append()`
...
{
  let fu = async move {
    let strm = unfold(Some((network, input)), move |state| {
