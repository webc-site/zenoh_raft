...
use validit::{Validate, less_equal};
  ...
  T: fmt::Display,
{
  ...
  pub(crate) fn accept(&mut self, new_accepted: T) {
    log::debug!("RAFT_io    {}; new_accepted: {}", self, new_accepted);
    Self::advance(&mut self.accepted, "accepted", new_accepted);
    log::debug!("RAFT_io    {}", self);
  }
  ...
  pub(crate) fn submit(&mut self, new_submitted: T) {
    log::debug!("RAFT_io    {}; new_submitted: {}", self, new_submitted);
  ...
  pub(crate) fn flush(&mut self, new_flushed: T) {
    log::debug!("RAFT_io    {}; new_flushed: {}", self, new_flushed);
