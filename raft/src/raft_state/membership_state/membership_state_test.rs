...
use crate::{
  BasicNode, Membership, StoredMembership,
  engine::testing::{UTConfig, UtClid, log_id},
  errors::{
    ChangeMembershipError, InProgress, NodeMetadataChanged, UnsupportedMembershipTransition,
  },
  raft_state::MembershipState,
  type_config::alias::{MembershipStateOf, StoredMembershipOf},
};
...
fn test_install_membership_snapshot_keeps_uncovered_effective() -> anyhow::Result<()> {
  ...
  let mut ms =
    MembershipStateOf::<UTConfig>::new(snapshot_membership.clone(), local_effective.clone());
...
fn test_validate_append_membership_unsupported_transition() -> anyhow::Result<()> {
  ...
  let want =
    ChangeMembershipError::UnsupportedMembershipTransition(UnsupportedMembershipTransition {
...
fn test_validate_append_membership_node_metadata() -> anyhow::Result<()> {
  ...
  let previous = Membership::new(
    voters(),
    btreemap! {1=>node("a"), 2=>node("b"), 3=>node("c")},
  )?;

  let ms = committed_node_state(previous);
  let before = ms.clone();

  let unchanged = Membership::new(
    voters(),
    btreemap! {1=>node("a"), 2=>node("b"), 3=>node("c")},
  )?;
  ...
  let voter_changed = Membership::new(
    voters(),
    btreemap! {1=>node("a"), 2=>node("b2"), 3=>node("c")},
  )?;
  ...
  let learner_changed = Membership::new(
    voters(),
    btreemap! {1=>node("a"), 2=>node("b"), 3=>node("c3")},
  )?;
