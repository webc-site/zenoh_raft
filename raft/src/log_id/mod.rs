...
#[derive(Debug, Clone, PartialOrd, Ord, PartialEq, Eq, bitcode::Encode, bitcode::Decode)]
  ...
  Term: RaftTerm,
{
  ...
  pub fn new_term_index(term: Term, index: u64) -> Self {
    LogId {
      leader_id: CommittedLeaderId::new(term),
      index,
    }
  }
}
...
#[cfg(test)]
mod tests {
  use crate::{log_id::raft_log_id::RaftLogId, vote::leader_id_std};
  ...
  fn test_new_term_index_equivalence() {
    ...
    assert_eq!(
      **log_id1.committed_leader_id(),
      **log_id2.committed_leader_id()
    );
  }
