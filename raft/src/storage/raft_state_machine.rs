...
use crate::{
  OptionalSend, OptionalSync, RaftSnapshotBuilder, RaftTypeConfig,
  storage::EntryResponder,
  type_config::alias::{LogIdOf, SnapshotMetaOf, SnapshotOf, StoredMembershipOf},
};
...
#[add_async_trait]
pub trait RaftStateMachine<C>: OptionalSend + OptionalSync + Unpin + 'static
where
  C: RaftTypeConfig,
{
  ...
  type SnapshotData: OptionalSend + Unpin + 'static;
...
fn unit_state_machine_error() -> io::Error {
  io::Error::other("unit type does not implement a usable state machine")
}
  ...
  C: RaftTypeConfig,
{
  ...
  async fn apply<Strm>(&mut self, _: Strm) -> Result<(), io::Error>
  where
    Strm: Stream<Item = Result<EntryResponder<C>, io::Error>> + Unpin + OptionalSend,
  {
    Err(unit_state_machine_error())
