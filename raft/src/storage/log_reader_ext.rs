...
use crate::{
  RaftLogReader, RaftTypeConfig, StorageError,
  entry::RaftEntry,
  errors::StorageIOResult,
  type_config::{TypeConfigExt, alias::LogIdOf},
};
  ...
  C: RaftTypeConfig,
{
  ...
  async fn try_get_log_entry(
    ...
    let mut res = self
      .try_get_log_entries(log_index..(log_index + 1))
      .await
      .sto_read_logs()?;
    Ok(res.pop())
  }
  ...
  async fn get_log_id(&mut self, log_index: u64) -> Result<LogIdOf<C>, StorageError<C>> {
    let entries = self
      .try_get_log_entries(log_index..=log_index)
      .await
      .sto_read_logs()?;
