...
use crate::{
  ...
  type_config::alias::{CommittedLeaderIdOf, EntryOf, LeaderIdOf, LogIdOf, VoteOf},
  vote::RaftVote,
};
    ...
    RB: RangeBounds<u64> + Clone + Debug + OptionalSend,
  {
    ...
    let fu = async move {
      ...
      if vote.leader_id() == &leader && vote.is_committed() {
        ...
        return Ok::<BoxStream<_>, _>(Box::pin(stream::iter([Err(changed_err(
          leader,
          Some(vote),
        ))])));
      }
    ...
    match fu.await {
      Ok(strm) => strm,
      Err(io_err) => Box::pin(stream::iter([Err(LeaderBoundedStreamError::IoError(
        io_err,
      ))])),
    }
  }
...
#[cfg(test)]
mod tests {
  use std::{collections::VecDeque, fmt::Debug, io::Error, ops::RangeBounds};

  use futures_util::StreamExt;

  use super::{LeaderBoundedStreamError, RaftLogReader};
  use crate::{
  ...
  impl RaftLogReader<UTConfig> for ChangingVoteReader {
    async fn try_get_log_entries<RB>(&mut self, _: RB) -> Result<Vec<EntryOf<UTConfig>>, Error>
    where
      RB: RangeBounds<u64> + Clone + Debug + crate::OptionalSend,
    {
      self.calls.push("read_entries");
      Ok(self.entries.clone())
    }

    async fn read_vote(&mut self) -> Result<Option<VoteOf<UTConfig>>, Error> {
