...
use crate::{
  ...
  raft_state::IOId,
  type_config::alias::{OneshotSenderOf, WatchSenderOf},
};
    ...
    match self {
      ...
      Self::Notify(IOFlushedNotify { io_id, tx }) => {
        let new_value = match result {
          ...
          Ok(_) => {
            log::debug!("{}: IOFlushed completed: {}", func_name!(), io_id);
            Ok(io_id)
          }
        };
        ...
        let modified = tx.send_if_modified(move |current| {
          if current.is_err() {
            ...
            log::debug!("IO completion ignored: already in error state");
            return false;
          }
        ...
        if !modified {
          log::debug!("IO completion not sent: channel state unchanged");
        }
      }
    }
  }
...
mod tests {
  ...
  fn test_io_flushed_notify_keeps_the_newest_io_id() {
    UTConfig::<()>::run(async {
      let term1 = IOId::<UTConfig>::new_log_io(
        Vote::new_committed(1, 1).to_committed(),
        Some(log_id(1, 1, 1)),
      );
      let term2 = IOId::<UTConfig>::new_log_io(
        Vote::new_committed(2, 1).to_committed(),
        Some(log_id(2, 1, 2)),
      );
