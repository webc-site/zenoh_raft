...
async fn test_zenoh_raft_cluster_over_quic_plain() -> Result<()> {
    ...
    for i in 1..=5 {
        raft0
            .client_write(ClientRequest::make_request("zenoh_plain_k", i))
            .await?;
    }
...
async fn test_zenoh_raft_cluster_over_quic_tls() -> Result<()> {
    ...
    for i in 1..=5 {
        raft0
            .client_write(ClientRequest::make_request("zenoh_tls_k", i))
            .await?;
    }
    ...
    for sm in [&sm0, &sm1, &sm2] {
        let sm_data = sm.get_state_machine().await;
        assert_eq!(
            sm_data.client_status.get("zenoh_tls_k").map(|s| s.as_str()),
            Some("request-5")
        );
    }
