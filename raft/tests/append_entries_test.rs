...
use maplit::btreeset;
use zenoh_raft::{
  Config, Entry, EntryPayload, RaftLogReader, ServerState, Vote,
  alias::EntryOf,
  raft::{AppendEntriesRequest, StreamAppendError, VoteRequest},
  storage::{RaftLogStorage, RaftLogStorageExt, RaftStateMachine},
  testing::{
    blank_ent, membership_ent,
    memstore::{ClientRequest, MemLogStore, TypeConfig},
  },
...
async fn test_append_inconsistent_log() -> Result<()> {
  ...
  for i in log_index + 1..=100 {
    sto0
      .blocking_append([blank_ent::<TypeConfig>(2, 1, i)])
      .await?;
    sto2
      .blocking_append([blank_ent::<TypeConfig>(3, 3, i)])
      .await?;
  }
